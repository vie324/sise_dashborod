...
export default async function handler(req, res) {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    return res.status(500).json({
      ...
      message: 'BLOB_READ_WRITE_TOKEN environment variable is not set. Please configure Vercel Blob Storage.',
    });
  }
  ...
  try {
    ...
    // Generate file path: photos/{folder}/{timestamp}_{fileName}.{ext}
